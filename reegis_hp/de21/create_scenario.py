...
def renewable_sources(df, time_series):
    ...
    for reg in df.index.get_level_values(0).unique():
        for vtype in ['solar', 'wind']:
            capacity = float(re_grouped.loc[reg].sum())
            ...
            if round(capacity) > 0:
                ...
                de21.add_sequences(idx, list(time_series[(vtype, reg)]))
...
def powerlines(df, efficiency=0.97):
    ...
    for line, values in df.iterrows():
        ...
        idx1 = ('LinearTransformer', label, source_bus, label)
        idx2 = ('LinearTransformer', label, label, target_bus)
        cols1 = ['conversion_factors', 'nominal_value', 'sort_index']
        cols2 = ['sort_index']
        values1 = [round(efficiency),
                   round(values['capacity']),
...
de21 = sc.SolphScenario(path=my_path, name=my_name)
de21.create_tables(year=year)
